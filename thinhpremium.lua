-- ⚙️ FAST CONFIG (FAST MỚI)
getgenv().FastAttackNew = true
getgenv().FAST_ON = true
getgenv().FAST_RATE = 0.01     -- mobile ổn: 0.04–0.07
getgenv().FAST_RANGE = 60

-- ❌ KHÓA TOÀN BỘ FAST CŨ (KHÔNG CẦN BIẾT TÊN)
if not getgenv().FastAttackNew then return end
...
local function W(p, n)
    local _, r = pcall(function() return p:WaitForChild(n) end)
    return r
end

local Enemies = W(Workspace, "Enemies")
local Characters = W(Workspace, "Characters")
local Modules = W(ReplicatedStorage, "Modules")
local Net = W(Modules, "Net")

DanhNhanhBuCu36.Rate = getgenv().FAST_RATE
...
function DanhNhanhBuCu36.PositionScan(ch, folder)
    ...
    for _, m in ipairs(folder:GetChildren()) do
        ...
        if hum and hrp and hum.Health > 0 then
            if (hrp.Position - pos).Magnitude <= getgenv().FAST_RANGE then
                table.insert(t, m)
            end
        end
    end
...
function DanhNhanhBuCu36.CollectAll(ch)
    local a = DanhNhanhBuCu36.PositionScan(ch, Enemies)
    local b = DanhNhanhBuCu36.PositionScan(ch, Characters)
    for _, v in ipairs(b) do table.insert(a, v) end
    return a
end

function DanhNhanhBuCu36.FastCore()
    if not getgenv().FAST_ON then return end
    ...
    local mobs = DanhNhanhBuCu36.CollectAll(ch)
    if #mobs == 0 then return end

    local parts = DanhNhanhBuCu36.HeadExtract(mobs)
    if #parts == 0 then return end

    Net["RE/RegisterAttack"]:FireServer(DanhNhanhBuCu36.Rate)
    Net["RE/RegisterHit"]:FireServer(parts[1][2], parts)
end

-- ===== BACKUP FAST (ANTI FIX) =====
local REMOTE_TARGET, REMOTE_ID
...
for _, f in ipairs({
    ...
    if f then
        for _, r in ipairs(f:GetChildren()) do
            if r:IsA("RemoteEvent") and r:GetAttribute("Id") then
                REMOTE_TARGET = r
                REMOTE_ID = r:GetAttribute("Id")
            end
        end
    end
end
...
function HacNoCu.Execute()
    ...
    local function Scan(folder)
        for _, mob in ipairs(folder:GetChildren()) do
            ...
            if hum and hr and hum.Health > 0 and mob ~= ch then
                if (hr.Position - HRP.Position).Magnitude <= getgenv().FAST_RANGE then
                    for _, p in ipairs(mob:GetChildren()) do
                        if p:IsA("BasePart") then
                            table.insert(gather, {mob, p})
                        end
                    end
                end
            end
    ...
    local tool = ch:FindFirstChildOfClass("Tool")
    if #gather > 0 and tool then
        Net["RE/RegisterAttack"]:FireServer()
        local head = gather[1][1]:FindFirstChild("Head")
        if head then
            Net["RE/RegisterHit"]:FireServer(head, gather, {})
        end
    end
end

-- ===== START (CHẠY NGẦM – KHÔNG CẦN UI) =====
task.spawn(function()
    while task.wait(getgenv().FAST_RATE) do
	...
	toggleButton.Image = "rbxassetid://130651380561716"
	...
	local Tabs = {
		...
		Mirage = Window:AddTab({
			Title = "island secret + up v4"
		}),
		Drago = Window:AddTab({
			Title = "island Drago Dojo"
		}),
	...
	local Players = game:GetService("Players")
	if World3 then
		Tabs.Main:AddSection("thịnh premium siêu bá")
...
spawn(function()
	...
	Tabs.Misc:AddButton({
		...
		Description = "",
		Callback = function()
			for i, v in pairs(game:GetObjects("rbxassetid://130651380561716")[1]:GetChildren()) do
